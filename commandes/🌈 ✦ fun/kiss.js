...
module.exports={
    name:"kiss",category:"🌈 ✦ fun",
    description:"To kiss someone",usage:"kiss <mention>",
        ...
        if(tools.category(message,"fun")==true){
            get("https://nekos.life/api/v2/img/kiss",(res)=>{
                ...
                res.on("end",()=>{
                    ...
                    if(tools.language(message)==0){
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**😘 KISS**').setDescription("<@"+message.author.id+"> kisses <@"+tools.user(message,args.join(" "),true).user.id+">"));
                    }else{
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**😘 KISS**').setDescription("<@"+message.author.id+"> embrasse <@"+tools.user(message,args.join(" "),true).user.id+">"));
                    };
                });
            }}).on("error",(error)=>{
