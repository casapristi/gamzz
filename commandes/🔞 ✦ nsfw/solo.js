...
module.exports={
    name:"solo",category:"🔞 ✦ nsfw",
    description:"To send a picture with the tag \"solo\"",
    run:async(client,message,args,tools)=>{
        if(tools.category(message,"nsfw")==true){
            get("https://nekos.life/api/v2/img/solo",(res)=>{
                ...
                res.on("end",()=>{
                    ...
                    if(tools.language(message)==0){
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**🔞 SOLO**'));
                    }else{
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**🔞 SOLO**'));
                    };
                });
