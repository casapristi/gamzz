...
module.exports={
    name:"erotic",aliases:["ero"],category:"🔞 ✦ nsfw",
    description:"To send an erotic picture",
    run:async(client,message,args,tools)=>{
        if(tools.category(message,"nsfw")==true){
            get("https://nekos.life/api/v2/img/ero",(res)=>{
                ...
                res.on("end",()=>{
                    ...
                    if(tools.language(message)==0){
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**🔞 EROTIC**'));
                    }else{
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**🔞 EROTIC**'));
                    };
                });
