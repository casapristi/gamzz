...
module.exports={
    name:"hentai",category:"🔞 ✦ nsfw",
    description:"To send a hentai picture",
    run:async(client,message,args,tools)=>{
        if(tools.category(message,"nsfw")==true){
            get("https://nekos.life/api/v2/img/hentai",(res)=>{
                ...
                res.on("end",()=>{
                    ...
                    if(tools.language(message)==0){
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**🔞 HENTAI**'));
                    }else{
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**🔞 HENTAI**'));
                    };
                });
