...
module.exports={
    name:"yuri",category:"🔞 ✦ nsfw",
    description:"To send a yuri picture",
        ...
        if(tools.category(message,"nsfw")==true){
            get("https://nekos.life/api/v2/img/yuri",(res)=>{
                ...
                res.on("end",()=>{
                    ...
                    if(tools.language(message)==0){
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**🔞 YURI**'));
                    }else{
                        message.channel.send(new MessageEmbed().setColor(tools.color(message)).setThumbnail(tools.icon(message)).setTimestamp().setImage(parsedData.url).setTitle('**🔞 YURI**'));
                    };
                });
